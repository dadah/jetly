  ...
  describe '.create' do
    ...
    context 'When shortening a valid url' do
      ...
      it 'should fetch a shortened url' do
        allow(JetlyUrl).to receive(:shorten).with(url_to_shorten).and_return(shortened_jetly_url)
        post :create, jetly_url: { complete_url: url_to_shorten }
        expect(assigns(:jetly_url)).to be(shortened_jetly_url)
      end
    end
